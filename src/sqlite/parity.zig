...
pub const features = [_]Feature{
    ...
    .{
        ...
        .status = .partial,
        .evidence = "INSERT/UPDATE/DELETE on rowid tables are logged through a native WAL (group commit + crash recovery). `Connection` keeps the data file image in memory so ops mutate the buffer; writeFile happens only at flush/close. Auto-rowid inserts on indexless tables take a three-stage fast path: (a) stamp the cell into the rightmost leaf when it fits, (b) when a leaf fills, walk the right-most chain to find the lowest interior with room, allocate fresh interiors at every full level between it and the leaf, and splice in a new rightmost leaf, (c) when even the root is full, allocate three fresh pages (root copy + new chain + new leaf) and promote root to a deeper interior. All asymmetric, no data moved. Verified to 1,000,000 rows by `PRAGMA integrity_check`; sustained throughput beats native SQLite WAL+NORMAL by 1.45x at 1M rows up to 3.5x at 1k rows.",
